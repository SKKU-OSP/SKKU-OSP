...
def teammember_options_exclude_members(user, team):
    try:
        ...
        usernames = list(Account.objects.exclude(user__is_superuser=True).exclude(
            user__id__in=team_members).values_list('user__username', flat=True))
...
def team_options_exclude_user(user, invite_user, recommend_team=-1):
    try:
        ...
        result = ''
        li1 = list(TeamMember.objects.filter(
            member=account).values_list('team_id', flat=True))
        li2 = list(TeamMember.objects.filter(
            member__user=invite_user).values_list('team_id', flat=True))
...
def get_admin_team(user):
    if not user.is_anonymous:
        team_li = list(TeamMember.objects.filter(
            member__user=user, is_admin=1).values_list("team_id", flat=True))
...
def get_team(user):
    if not user.is_anonymous:
        team_li = list(TeamMember.objects.filter(
            member__user=user).values_list("team_id", flat=True))
        return Team.objects.filter(id__in=team_li)
    return None
