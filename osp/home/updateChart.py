...
for case in range(4):
    ...
    # print("scoreAnnual ", scoreAnnual)
    # print("scoreSubAnnual", scoreSubAnnual)
    ...
    for ele in result:
        ...
        if Row['issue_count'] > 0 :
            ...
            offset = 1
            #print("unsort ",idx1,idxDept,issueDeptTop5pct[idx1][idxDept], Row['issue_count'])
            if list_len <= 0:
    ...
    # print("#################")
    # print("top print")
    # print(scoreSidTop5pct[0])
    # print(scoreSubSidTop5pct[0])
    # print(scoreSubSidTop5pct[1])
    # print(scoreSubSidTop5pct[2])
    # print(issueDeptTop5pct[0])
    # print(forkDeptTop5pct[0])
    ...
    for i in range(nYear):
        for j in range(len(scoreSid[i])):
            # print(scoreSid[i][j], sidSize[i][j])
            if sidSize[i][j] == 0:
                continue
    ...
    for i in range(nYear):
        for j in range(7) :
            if sidSize[i][j] == 0:
                scoreSidStd[i][j] = math.nan
                scoreSubSidStd[i][j] = math.nan
                scoreSumSidStd[i][j] = math.nan
                commitSidStd[i][j] = math.nan
                starSidStd[i][j] = math.nan
                prSidStd[i][j] = math.nan
                issueSidStd[i][j] = math.nan
                forkSidStd[i][j] = math.nan
                scoreSidStd[i][j] = math.nan
            else:
    ...
    for i in range(nYear) :
        for j in range(3) :
            if sidSize[i][j] == 0:
                scoreDeptStd[i][j] = math.nan
                scoreSubDeptStd[i][j] = math.nan
                scoreSumDeptStd[i][j] = math.nan
                commitDeptStd[i][j] = math.nan
                starDeptStd[i][j] = math.nan
                prDeptStd[i][j] = math.nan
                issueDeptStd[i][j] = math.nan
                forkDeptStd[i][j] = math.nan
            else:
    ...
    for i in range(nYear) :
        if sidSize[i] == 0:
            scoreYearStd[i] = math.nan
            scoreSubYearStd[i] = math.nan
            scoreSumYearStd[i] = math.nan
            commitYearStd[i] = math.nan
            starYearStd[i] = math.nan
            prYearStd[i] = math.nan
            issueYearStd[i] = math.nan
            forkYearStd[i] = math.nan
        else:
    ...
    # # MODEL2: insert_annualoverview_sql
    # try:
    #     insert_annualtotal_sql = """insert into home_annualtotal 
    #     (case_num, student_KP, student_KP_diff, student_KP_sum, commit, star, repo, repo_total) 
    #     VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
        
    #     cursor.execute(insert_annualtotal_sql, 
    #         (case, json.dumps(scoreMore3), json.dumps(scoreSubMore3), json.dumps(scoreSumMore3), json.dumps(totalCommit), json.dumps(totalStar), json.dumps(repo_dist), 
    #         repo_total))
    #     conn.commit()
    # except Exception as e:
    ...
    #     score_sid, score_sid_diff, score_sid_sum, score_sid_std, score_sid_std_diff, score_sid_std_sum, score_sid_pct,
    #     score_dept, score_dept_diff, score_dept_sum, score_dept_std, score_dept_std_diff, score_dept_std_sum, score_dept_pct) 
    #     VALUES (%s, %s, %s, %s, %s, 
    #     %s, %s, %s, %s, %s, %s, %s,
    #     %s, %s, %s, %s, %s, %s, %s)"""
    ...
    #         json.dumps(scoreSid[yid]), json.dumps(scoreSubSid[yid]), json.dumps(scoreSumSid[yid]), json.dumps(scoreSidStd[yid]), json.dumps(scoreSubSidStd[yid]), json.dumps(scoreSumSidStd[yid]), json.dumps(scoreSidTop5pct[yid]), 
    #         json.dumps(scoreDept[yid]), json.dumps(scoreSubDept[yid]), json.dumps(scoreSumDept[yid]), json.dumps(scoreDeptStd[yid]), json.dumps(scoreSubDeptStd[yid]), json.dumps(scoreSumDeptStd[yid]), json.dumps(scoreDeptTop5pct[yid])))
    #     conn.commit()
    # except Exception as e:
