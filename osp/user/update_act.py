...
def update_commmit_time():
    ...
    for i in range(len(stdnt_cmt_time.index)):
        ...
        if row.student_github not in stdnt_cmt_hour:
            stdnt_cmt_hour[row.student_github] = {"student_github":row.student_github, "hour_dist":[0]*24, "daytime":0, "night":0}
    ...
    circmean_total = circmean(circmean_src, high=86399, low=0, axis=None, nan_policy='propagate')
    ...
    student_commits_time['committer_hour_circmean'] = student_commits_time['committer_time_circmean'].map(lambda x: int(x/3600))

    student_commits_time.reset_index().to_csv(DATA_DIR + commmit_time_path2, index=False)
    ...
    time_sector = pd.DataFrame({'sector' : ['major_min','major_max', 'daytime_min', 'daytime_max', 'total_circmean', 'freq_hour'], 'second': [sectors[max_index-radius], sectors[max_index+radius+1], daytime_min*3600, daytime_max*3600, int(circmean_total), max_total_index]})
...
def update_individual():
    ...
    result_df.loc[:, ['individual', 'group', 'major_act']].to_csv(DATA_DIR + major_act_path1)
...
def update_frequency():
    ...
    df_id_result.loc[:,['id', 'type3', "dist"]].to_csv(DATA_DIR + frequency_path1, index=False)
...
def read_commit_time(username):
    ...
    try:
        ...
        print("Read Hour_dist error", e)
        hour_dist = [0]*24
        daytime = 0
        night= 0
    try:
        committer_time_circmean = pd.read_csv(filepath2, index_col = 0)
        time_circmean = committer_time_circmean.at[username, "committer_time_circmean"]
    except Exception as e:
        print("Read Circmean error", e)
        time_circmean = 0
    ...
    try:
        ...
        print("Read Time_sector error", e)
        daytime_min = 32400
        daytime_max = 75600
...
def read_major_act(username):
    ...
    try:
        ...
        print("Read Major_act error", e)
        major_act = "individual"
        indi_num = 0
        group_num = 0
...
def read_frequency(username):
    ...
    try:
        ...
        print("Read Freq error", e)
        commit_freq = 0
        commit_freq_dist = [0]*4
