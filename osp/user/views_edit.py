...
class ProfileEditView(TemplateView):
    ...
    def get_context_data(self, request, *args, **kwargs):
        ...
        # 타인이 edit페이지 접속 시도시 프로필 페이지로 돌려보냄
        if str(request.user) != context["username"]:
        ...
        lang_tags = TagIndependent.objects.filter(name__in=AccountInterest.objects.filter(
            account=student_account).exclude(tag__type="domain").values("tag")).order_by("name")
        account_lang = AccountInterest.objects.filter(
            account=student_account, tag__in=lang_tags).exclude(tag__type="domain").order_by("tag__name")
        ...
        context["ints"] = AccountInterest.objects.filter(
            account=student_account).filter(tag__in=tags_domain)
        context["lang"] = AccountInterest.objects.filter(
            account=student_account).exclude(tag__in=tags_domain)
        context["privacy"] = AccountPrivacy.objects.get(
...
class ProfileInterestsView(UpdateView):
    def post(self, request, username, *args, **kwargs):
        ...
        if request.POST['act'] == 'append':
            added_preferLanguage = request.POST.get(
                'interestDomain')  # 선택 된 태그
            added_tag = TagIndependent.objects.get(name=added_preferLanguage)
            try:
                already_ints = AccountInterest.objects.get(
                    account=user_account, tag=added_tag)
                already_ints.delete()
                AccountInterest.objects.create(
                    account=user_account, tag=added_tag, level=0)
            except:
                AccountInterest.objects.create(
                    account=user_account, tag=added_tag, level=0)

        else:
            delete_requested_tag = TagIndependent.objects.get(
                name=request.POST['target'])
            tag_deleted = AccountInterest.objects.get(
                account=user_account, tag=delete_requested_tag).delete()
    ...
    def post(self, request, username, *args, **kwargs):
        ...
        def langupdater(tier, level):
            for lang, val in tier.items():
                lang_tag = TagIndependent.objects.get(name=val.replace(
                    "_", " ").replace("plus", "+").replace("sharp", "#"))
                new_interest_obj = AccountInterest(
                    account=user_account, tag=lang_tag, level=level)
        ...
        def intsupdater(ints):
            ...
            for intt, val in ints.items():
                int_tag = TagIndependent.objects.get(name=val.replace(
                    "_", " ").replace("plus", "+").replace("sharp", "#"))
                print(int_tag)
                new_interest_obj = AccountInterest(
                    account=user_account, tag=int_tag)
                query_bulk.append(new_interest_obj)
