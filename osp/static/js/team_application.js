teamApplication = {
  ...
  applyResult: function (team_id, username, is_okay) {
    ...
    let status = is_okay ? '수락' : '거절';
    let confirmMsg = username + ' 님의 팀 지원을 ' + status + '하시겠습니까?';
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data['status'] == 'success') {
          console.log('target', `#apply-user-${data['username']}-${data['team_id']}`);
          $(`#apply-user-${data['username']}-${data['team_id']}`).remove();
