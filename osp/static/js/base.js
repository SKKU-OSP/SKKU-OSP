function message_box_builder(msg, is_receive) {
  ...
  var msg_box = $('<div></div>')
    .append(
      $('<div></div>')
        .addClass('chat-text')
        .append($('<div></div>').addClass('chat-text-body').html(msg.body))
        .append($('<div></div>').addClass(msg.read == 'True' ? '' : 'chat-unread'))
    )
    .append(
      $('<div></div>')
        .addClass('chat-date')
        .html(
          msg_date.toLocaleString('en-US', {
            month: 'numeric',
            day: 'numeric',
            hour: 'numeric',
            hour12: true,
  ...
  $('#chat-view')
    ...
    .append(
      $('<div></div')
        ...
        .addClass('m-auto')
        .append($('<span></span>').addClass('visually-hidden').html('Loading...'))
    );
...
/**
 * 유저의 모든 알림 메시지를 읽음 처리하는 함수
 */
function readAllNotification() {
  $.ajax({
    type: 'POST',
    url: '/message/noti-read/',
    dataType: 'JSON'
  })
    .done(function (data) {
      $('.noti-item').addClass('read');
      $('.noti-item').removeAttr('onclick');
      $('#new-noti').css('display', 'none');
    })
    .fail(function (data) {
