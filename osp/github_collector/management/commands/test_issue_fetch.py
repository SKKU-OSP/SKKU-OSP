...
from github_collector.collectors.issue import IssueCollector
from github_collector.api.client import GithubApiClient
from github_collector.services.issue_service import save_issues_in_batch


class Command(BaseCommand):
    help = "특정 레포지토리의 특정 사용자의 GitHub Issue를 수집하고 저장합니다."
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            issue_gen = collector.find_all_issues(owner, repo, username, start_date, end_date)
            ...
            issue_gen = collector.find_all_issues(owner, repo, username, start_date, end_date)
            save_issues_in_batch(issue_gen)
            
            self.stdout.write(f"Issue 수집 완료: {owner}/{repo} - {username}")
